...
(function(d, a, c) {
    ...
    b.jsonp = function(s) {
        var q = false, l = function() {}, h = b.random.name("PICARD_UTILS_JSONP_CALLBACK"), e = s.url, i = b.obj.copy(s.data), m = s.callbackName || "callback", p = s.callback || l, f = s.success || l, j = s.error || l, o = s.complete || l, r = a.getElementsByTagName("script")[0], k = a.createElement("script");
        i[m] = "window." + h;
...
(function(window, document, kik) {
    ...
    function setupPlugin(pluginName) {
        var pluginObj = kik.events(), pluginData = makeBridgeCall(PLUGIN_REQUEST_NAME, {
            name: pluginName,
    ...
    function generatePluginBatchSegment(data, callback) {
        var pluginObj = kik.events(), argData = JSON.stringify({
            name: data.name,
            eventCallback: setupEventCallback(pluginObj.trigger)
        });
        var callbackName = kik.utils.random.name("PICARD_BRIDGE_CALLBACK");
...
(function(k, q, r) {
    function v(F, J) {
        var E = k.applicationCache, I = false;
        ...
        if ((E.status !== E.IDLE) && (E.status !== E.CHECKING) && (E.status !== E.DOWNLOADING)) {
            J(false);
            return 
        }
    ...
    var C = {};
    if (a.setCardInfo) {
        var o = /(^|\s)icon(\s|$)/i, z, x, n, g, B, i;
        Array.prototype.forEach.call(q.getElementsByTagName("link"), function(D) {
            if (o.test(D.rel)) {
                z = D.href
            } else {
                if (D.rel === "kik-icon") {
    ...
    if (a.setStatusBarTransparent) {
        ...
        var u;
        Array.prototype.forEach.call(q.getElementsByTagName("meta"), function(D) {
    ...
    if (a.getOrientationLock && a.setOrientationLock) {
        ...
        A.setOrientationLock = function(D) {
            ...
            default:
                ...
                throw TypeError("if defined, position " + D + ' must be one of "free", "portrait", or "landscape"')
            }
            try {
    ...
    if (a.getLastLinkData) {
        s(r._.firstBatch["Browser.getLastLinkData"] || a.getLastLinkData(), false)
    }
    a.on("linkData", s);
    var p = true, y = false, w = true;
    A.background = p;
